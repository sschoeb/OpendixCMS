 ...
 *
 */
class Filebrowser extends ModulBase
{
	...
	public function __construct()
	{
		...
		CssImport::ImportCss ( 'filebrowser.css' );
		
		if (! isset ( $_SESSION ['filebase_dirs'] ))
		{
			$_SESSION ['filebase_dirs'] = '';
			$_SESSION ['filebase_files'] = '';
		}
		...
		if (! isset ( $_SESSION ['filebase_temp'] ))
			$_SESSION ['filebase_temp'] = '';
		
		if (isset ( $_GET ['action'] ) && (! isset ( $_GET ['nextaction'] ) || $_SESSION ['nextaction'] == $_GET ['nextaction']))
		{
			switch ($_GET ['action'])
			{
				case 'root' :
		...
		MySmarty::GetInstance ()->OutputModuleVar ( 'createFolderLink', Functions::GetLink ( array ('sub' => $_GET ['sub'], 'action' => 'createFolder', 'nextaction' => md5 ( time () ) ) ) );
		MySmarty::GetInstance ()->OutputModuleVar ( 'uploadLink', Functions::GetLink ( array ('sub' => $_GET ['sub'], 'action' => 'upload', 'nextaction' => md5 ( time () ) ) ) );
		$this->showDir ();
	}
	...
	private function CreateFolder()
	{
		...
		if (is_dir ( $fileBaseFolder . $_SESSION ['filebase_temp'] . $folderName ))
		{
			throw new CMSException ( 'Ordner existiert bereits - Nicht angelegt!', CMSException::T_WARNING );
		}
		if (! @mkdir ( $fileBaseFolder . $_SESSION ['filebase_temp'] . $folderName, 0777 ))
		{
	...
	private function outputNavString()
	{
		...
		MySmarty::GetInstance ()->OutputModuleVar ( 'rootLink', Functions::GetLink ( array ('sub' => $_GET ['sub'], 'action' => 'root' ) ) );
		
		//Links
		$arr = preg_split ( '/\//', substr ( $_SESSION ['filebase_temp'], 0, strlen ( $_SESSION ['filebase_temp'] ) - 1 ) );
		...
		{
			$out [] ['name'] = $arr [$i] . ' /';
			$arr [$i] = $fileBaseFolder . $arr [$i];
			$out [count ( $out ) - 1] ['link'] = Functions::getlink ( array ('sub' => $_GET ['sub'], 'action' => 'joinFolder', 'path' => "$i" ) );
		}
		
		MySmarty::GetInstance ()->OutputModuleVar ( 'filebase_navLink', $out );
	}
	...
	private function DeleteFile()
	{
		...
		if (! Filebase::removeFile ( $filebasePath . $_SESSION ['filebase_temp'] . $files [$_GET ['path']] ))
		{
	...
	private function ShowDir()
	{
		...
		{
			MySmarty::GetInstance ()->OutputModuleVar ( 'filesBackLink', Functions::GetLink ( array ('sub' => $_GET ['sub'], 'action' => 'up', 'nextaction' => md5 ( time () ) ) ) );
		}
		...
		MySmarty::GetInstance ()->OutputModuleVar ( 'directories', $dirs );
		...
		MySmarty::GetInstance ()->OutputModuleVar ( 'files', $files );
	...
	{
		$fileBaseFolder = Cms::GetInstance ()->GetConfiguration ()->Get ( 'Path/filebaseFolder' );
		if (! $path = Filebase::UploadFile ( $_SESSION ['filebase_temp'], 'fileUpload' ))
		{
