...
class Smarty_Compiler extends Smarty {
    ...
    public $_init_smarty_publics      =   false;
    ...
    public $_public_bracket_regexp    =   null;
    public $_num_const_regexp      =   null;
    public $_dpublic_guts_regexp      =   null;
    public $_dpublic_regexp           =   null;
    public $_cpublic_regexp           =   null;
    public $_spublic_regexp           =   null;
    public $_apublic_regexp           =   null;
    public $_mod_regexp            =   null;
    public $_public_regexp            =   null;
    ...
    function _compile_foreach_start($tag_args)
    {
       ...
       /* if (isset($name)) {
        ...
        if (isset($name)) { 
            ...
            $output .= "{$foreach_props} = array('total' => count(\$_from), 'iteration' => 0);\n"; 
            $output .= "if ({$foreach_props}['total'] > 0):\n"; 
            $output .= "    foreach (\$_from as \$_".md5($key_part)." => \$_".md5($item)."):\n"; 
            $output .= "        {$foreach_props}['iteration']++;\n"; 
            $output .= ($key_part) ? "        \$this->_tpl_vars['$key'] = \$_".md5($key_part).";\n" : '' ; 
            $output .= '        $this->_tpl_vars[\''.$item.'\'] = $_'.md5($item).";\n"; 
        } else { 
            $output .= "if (count(\$_from)):\n"; 
            $output .= "    foreach (\$_from as \$_".md5($key_part)." => \$_".md5($item)."):\n"; 
            $output .= ($key_part) ? '        '.$key_part.' = $_'.md5($key_part).";\n" : ''; 
            $output .= '        $this->_tpl_vars[\''.$item.'\'] = $_'.md5($item)."\n"; 
        }      
